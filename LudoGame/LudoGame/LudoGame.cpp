...
void main()
{
    ...
    A.NoT = 0; B.NoT = 0; C.NoT = 0; D.NoT = 0; char con;
    ...
    for (n = 16; n < 88; n++)
        Ck[n][0] = 0; Ck[n][1] = 0; Ck[n][2] = 0; Ck[n][3] = 0;
    strcpy_s(Name[0], "Computer");
    strcpy_s(Name[1], "Computer");
    strcpy_s(Name[2], "Computer");
    strcpy_s(Name[3], "Computer");
    rep[0] = 0; rep[1] = 0; rep[2] = 0; rep[3] = 0;
    F[0] = 88; F[1] = 92; F[2] = 96; F[3] = 100;
    cout << "\t\t\t" << "WELCOME TO LUDO GAME!!!!!!!!!" << "\n\n";
    cout << "# @ & % are the tokens" << "\n\n";
    cout << "Enter Number of players: "; cin >> play;    // To Decide the no: of players 
    cout << endl;
    while (play<0 || play>4)
    {
        cout << "Plz Enter Number of plyaers between (1-4) : "; cin >> play;
    }
    int select[4] = { 0,0,0,0 };
    for (i = 0; i < play; i++)
    {
        cout << "Enter Name of the Player " << i + 1 << " : "; cin >> name;
        retry:
        cout << "Choose Your Token: "; cin >> tok;
        if (tok == '#' && select[0]==0)
        {
            A.placom = 1; strcpy_s(Name[0], name);
            select[0] = 1;
        }  // This will decide that 
        else if (tok == '@'&& select[1] == 0)                           // who is player and who is computer 
        {
            B.placom = 1; strcpy_s(Name[1], name);
            select[1] = 1;
        }
        else if (tok == '&' && select[2] == 0)
        {
            C.placom = 1; strcpy_s(Name[2], name);
            select[2] = 1;
        }
        else if (tok == '%' && select[3] == 0)
        {
            D.placom = 1; strcpy_s(Name[3], name);
            select[3] = 1;
        }
        else
        {
            cout << "Choose Correct one " << endl << endl;
            goto retry;
        }
        cout << endl;
    }
    ...
    again:
        ...
        if (Z == 0)
        {
            ...
            Turn(Num, Ludo);
            if (Num >= 1 && Num <= 5)     // Logics made for Ludo 
            {
                if (A.NoT == 0)
                {
                    Z++; 
